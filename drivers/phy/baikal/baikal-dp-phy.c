 ...
 */
#define PHY_PMA_CMN_READY		0x10
#define PHY_CFG0_LN(ln) 		(0x20 + 0x30 * (ln))
#define PMA_XCVR_POWER_STATE_REQ_SHIFT	2
#define PMA_XCVR_POWER_STATE_REQ_MASK	GENMASK(7, 2)
#define PMA_XCVR_PLLCLK_EN		BIT(1)
#define PMA_TX_ELEC_IDLE		BIT(0)
#define PHY_CFG2_LN(ln) 		(0x40 + 0x30 * (ln))
#define PMA_XCVR_POWER_STATE_ACK_SHIFT	1
#define PMA_XCVR_POWER_STATE_ACK_MASK	GENMASK(6, 1)
#define PMA_XCVR_PLLCLK_EN_ACK		BIT(0)
#define PMA_CMN_REFCLK_REG0		0xe0
#define PMA_CMN_REFCLK_REG0_CMN_REFCLK_MODE_MASK	GENMASK(4, 3)
#define PMA_CMN_REFCLK_REG0_CMN_REFCLK_MODE_SHIFT	3
#define PMA_CMN_REFCLK_REG0_CMN_REFCLK_INT_MODE_MASK	GENMASK(6, 5)
#define PMA_CMN_REFCLK_REG0_CMN_REFCLK_INT_MODE_SHIFT	5
#define PMA_CMN_REFCLK_REG0_CMN_REFCLK_DIG_DIV_MASK	GENMASK(9, 8)
#define PMA_CMN_REFCLK_REG0_CMN_REFCLK_DIG_DIV_SHIFT	8
#define PHY_LN_BUF_CFG			0x100
#define PHY_LN_BUF_CFG_LN_0_RSTN	BIT(1)
#define PHY_LN_BUF_CFG_LN_1_RSTN	BIT(4)
#define PHY_LN_BUF_CFG_LN_2_RSTN	BIT(9)
#define PHY_LN_BUF_CFG_LN_3_RSTN	BIT(12)

static const u32 lane_reset_bit[] = {
	PHY_LN_BUF_CFG_LN_0_RSTN,
	PHY_LN_BUF_CFG_LN_1_RSTN,
	PHY_LN_BUF_CFG_LN_2_RSTN,
	PHY_LN_BUF_CFG_LN_3_RSTN
};
				      ...
				      bool enable)
{
	...
	u32 pll_val = cdns_torrent_dp_read(regmap, PHY_CFG0_LN(0));

	if (enable) {
		pll_val |= PMA_XCVR_PLLCLK_EN;
		pll_ack_val = PMA_XCVR_PLLCLK_EN_ACK;
	} else {
		pll_val &= ~PMA_XCVR_PLLCLK_EN;
		pll_ack_val = 0;
	}

	cdns_torrent_dp_write(regmap, PHY_CFG0_LN(0), pll_val);

	/* Wait for acknowledgment from PHY. */
	ret = regmap_read_poll_timeout(regmap,
			       PHY_CFG2_LN(0),
			       rd_val,
			       (rd_val & PMA_XCVR_PLLCLK_EN_ACK) == pll_ack_val,
			       0, POLL_TIMEOUT_US);
	ndelay(100);
	return ret;
}
					   ...
					   enum phy_powerstate powerstate)
{
	...
	value = cdns_torrent_dp_read(regmap, PHY_CFG0_LN(0));
	value &= ~PMA_XCVR_POWER_STATE_REQ_MASK;
	value |= value_part << PMA_XCVR_POWER_STATE_REQ_SHIFT;
	ack_mask = PMA_XCVR_POWER_STATE_ACK_MASK;
	ack_value = value_part << PMA_XCVR_POWER_STATE_ACK_SHIFT;
	/* Set power state A<n>. */
	cdns_torrent_dp_write(regmap, PHY_CFG0_LN(0), value);
	/* Wait, until PHY acknowledges power state completion. */
	ret = regmap_read_poll_timeout(regmap, PHY_CFG2_LN(0),
			       read_val, (read_val & ack_mask) == ack_value, 0,
			       POLL_TIMEOUT_US);
	if (ret)
		return ret;
			       ...
			       struct cdns_torrent_inst *inst, u32 num_lanes)
{
	 ...
	 */
	ret = regmap_read_poll_timeout(regmap, PHY_CFG2_LN(0),
				       read_val, read_val & PMA_XCVR_PLLCLK_EN_ACK,
				       0, POLL_TIMEOUT_US);
	if (ret == -ETIMEDOUT) {
				       ...
				       u32 num_lanes)
{
	...
	reg = cdns_torrent_dp_read(regmap, PHY_CFG0_LN(inst->mlane + 0));
	reg &= ~PMA_XCVR_POWER_STATE_REQ_MASK;
	reg &= ~PMA_XCVR_PLLCLK_EN;
	cdns_torrent_dp_write(regmap, PHY_CFG0_LN(inst->mlane + 0), reg);
}
				     ...
				     struct phy_configure_opts_dp *dp)
{
	...
	for (i = 0; i < dp->lanes; i++) {
		value = cdns_torrent_dp_read(regmap, PHY_CFG0_LN(i));
		if (i < clane || i >= clane + inst->num_lanes)
			value |= PMA_TX_ELEC_IDLE;
		else
			value &= PMA_TX_ELEC_IDLE;
		cdns_torrent_dp_write(regmap, PHY_CFG0_LN(i), value);
	}

	/* reset the link by asserting master lane phy_l0*_reset_n low */
	value = cdns_torrent_dp_read(regmap, PHY_LN_BUF_CFG);
	value &= ~lane_reset_bit[clane];
	cdns_torrent_dp_write(regmap, PHY_LN_BUF_CFG, value);
	...
	// TODO probably needs to be refactored
	for (i = 0; i < dp->lanes; i++) {
		if (i < clane || i >= clane + inst->num_lanes)
			value &= ~lane_reset_bit[i];
		else
	...
	value = cdns_torrent_dp_read(regmap, PHY_CFG0_LN(clane));
	value |= PMA_XCVR_PLLCLK_EN;
	cdns_torrent_dp_write(regmap, PHY_CFG0_LN(clane), value);
...
static int cdns_torrent_phy_on(struct phy *phy)
{
	...
	if (cdns_phy->nsubnodes == 1) {
		...
		ret = reset_control_deassert(inst->lnk_rst);

		/* Set initial power state */
		value = cdns_torrent_dp_read(regmap, PHY_CFG0_LN(inst->mlane));
		value &= ~PMA_XCVR_POWER_STATE_REQ_MASK;
		//ack_mask = PMA_XCVR_POWER_STATE_ACK_MASK;
		//ack_value = value_part << PMA_XCVR_POWER_STATE_ACK_SHIFT;
		/* Set power state A<n>. */
		cdns_torrent_dp_write(regmap, PHY_CFG0_LN(inst->mlane), value);
		ndelay(100);
					...
					struct cdns_torrent_inst *inst)
{
	 ...
	 * used lanes
	 */
	// TODO keep in mind that here, as in vanilla driver, it is assumed
	// that master lane is always 0 - is it correct?
	reset_val = cdns_torrent_dp_read(regmap, PHY_LN_BUF_CFG);
	for (int i = 0; i < inst->num_lanes; i++) {
		val = cdns_torrent_dp_read(regmap, PHY_CFG0_LN(i));
		val &= PMA_TX_ELEC_IDLE;
		cdns_torrent_dp_write(regmap, PHY_CFG0_LN(i), val);
		reset_val |= lane_reset_bit[i];
	}
	cdns_torrent_dp_write(regmap, PHY_LN_BUF_CFG, reset_val);

	/* release pma_xcvr_pllclk_en_ln_*, only for the master lane */
	val = cdns_torrent_dp_read(regmap, PHY_CFG0_LN(0));
	val &= ~PMA_XCVR_PLLCLK_EN;
	cdns_torrent_dp_write(regmap, PHY_CFG0_LN(0), val);
...
static int cdns_torrent_clk(struct cdns_torrent_phy *cdns_phy)
{
	...
	case REF_CLK_156_25MHZ:
		...
		break;
	default:
		dev_err(cdns_phy->dev, "Invalid ref clock rate %lu\n", ref_clk_rate);
		ret = -EINVAL;
		goto disable_clk;
	}
...
static int baikal_dp_phy_probe(struct platform_device *pdev)
{
	...
	for_each_available_child_of_node(dev->of_node, child) {
		...
		if (cdns_phy->phys[node].phy_type == TYPE_DP) {
			...
			uint32_t ref_clk_rate = cdns_phy->ref_clk_rate;
		        unsigned int refclk_mode = (ref_clk_rate >= CLK_100_MHZ) ? 0x0 : 0x2; // differential(!) reference clock
		        unsigned int refclk_int_mode = (ref_clk_rate >= CLK_100_MHZ) ? 0x1 : 0x3;
		        unsigned int refclk_dig_div = 0;
		        if (ref_clk_rate >= CLK_19_2_MHZ && ref_clk_rate <= CLK_25_MHZ)
				refclk_dig_div = 0;             // divide by 1 (reference clock 19.2-27 MHz)
		        else if (ref_clk_rate == CLK_100_MHZ)
				refclk_dig_div = 2;             // divide by 4 (reference clock 100 MHz)
		        else
				return -1;

			struct regmap *regmap = cdns_phy->regmap_dptx_phy_reg;
		        uint32_t value = cdns_torrent_dp_read(regmap, PMA_CMN_REFCLK_REG0);
			value &= ~(PMA_CMN_REFCLK_REG0_CMN_REFCLK_MODE_MASK | PMA_CMN_REFCLK_REG0_CMN_REFCLK_INT_MODE_MASK | PMA_CMN_REFCLK_REG0_CMN_REFCLK_DIG_DIV_MASK);
		        value |= refclk_mode << PMA_CMN_REFCLK_REG0_CMN_REFCLK_MODE_SHIFT;
		        value |= refclk_int_mode << PMA_CMN_REFCLK_REG0_CMN_REFCLK_INT_MODE_SHIFT;
		        value |= refclk_dig_div << PMA_CMN_REFCLK_REG0_CMN_REFCLK_DIG_DIV_SHIFT;
		        cdns_torrent_dp_write(regmap, PMA_CMN_REFCLK_REG0, value);

			usleep_range(1000, 1100);

		        value = cdns_torrent_dp_read(regmap, PMA_CMN_REFCLK_REG0);
	...
	// TODO APB reset moved here
	/* Enable APB */
	ret = reset_control_deassert(cdns_phy->apb_rst);
...
static void baikal_dp_phy_remove(struct platform_device *pdev)
{
...
static DEFINE_NOIRQ_DEV_PM_OPS(baikal_dp_phy_pm_ops,
...
static const struct cdns_torrent_data baikal_map_torrent = {
...
static const struct of_device_id baikal_dp_phy_of_match[] = {
	{
		.compatible = "baikal,dp-phy",
		.data = &baikal_map_torrent,
	},
	{}
};
...
static struct platform_driver baikal_dp_phy_driver = {
	...
	.remove_new = baikal_dp_phy_remove,
	.driver = {
		.name	= "baikal-dp-phy",
		.of_match_table	= baikal_dp_phy_of_match,
		.pm	= pm_sleep_ptr(&baikal_dp_phy_pm_ops),
	}
};
module_platform_driver(baikal_dp_phy_driver);
