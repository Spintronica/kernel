...
static unsigned int pvt_processing_mode = CONFIG_SENSORS_BAIKAL_PVT_MODE;

static int param_set_processing_mode(const char *val, const struct kernel_param *kp)
{
	return param_set_uint_minmax(val, kp, 1, 3);
}
...
static enum pvt_sensor_type pvt_sensor_last;
				    ...
				    u32 attr, int ch)
{
	...
	case hwmon_in:
		...
		case hwmon_in_input:
		case hwmon_in_label:
			if ((pvt_processing_mode > 1 && ch == 0) ||
			    pvt_processing_mode > 2)
				return 0444;
			break;
		case hwmon_in_min:
		case hwmon_in_max:
...
#if defined(CONFIG_SENSORS_BAIKAL_PVT_ALARMS)
	kt = ktime_divns(kt, pvt_sensor_last + 1);
#endif
...
#if defined(CONFIG_SENSORS_BAIKAL_PVT_ALARMS)
	pvt_sensor_last = pvt_processing_mode > 2 ? PVT_SENSOR_LAST :
			  pvt_processing_mode > 1 ? PVT_VOLT : PVT_SENSOR_FIRST;
...
static int pvt_request_clks(struct pvt_hwmon *pvt)
{
	...
	if (!pvt->clks[PVT_CLOCK_APB].id)
		pvt->clks[PVT_CLOCK_APB].id = "pclk";
	if (!pvt->clks[PVT_CLOCK_REF].id)
...
static int pvt_init_iface(struct pvt_hwmon *pvt)
{
	 ...
	 */
	pvt->timeout = ktime_set(0, PVT_TOUT_DEF * (pvt_sensor_last + 1));
	tout = pvt_calc_tout(
#if defined(CONFIG_SENSORS_BAIKAL_PVT_ALARMS)
		ktime_divns(pvt->timeout, pvt_sensor_last + 1)
#else
		pvt->timeout
#endif
		, rate);
