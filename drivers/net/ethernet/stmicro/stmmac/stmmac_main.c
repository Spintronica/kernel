				   ...
				   u32 queue, gfp_t flags)
{
	...
	for (i = 0; i < dma_conf->dma_rx_size; i++) {
		...
		np = STMMAC_NEXT_ENTRY(i, dma_conf->dma_rx_size);
		if (priv->extend_desc) {
				    ...
				    u32 queue)
{
	...
	for (i = 0; i < dma_conf->dma_tx_size; i++) {
		...
		np = STMMAC_NEXT_ENTRY(i, dma_conf->dma_tx_size);
		if (priv->extend_desc) {
			p = &tx_q->dma_etx[i].basic;
			   ...
			   bool *pending_packets)
{
	...
	while ((entry != tx_q->cur_tx) && count < priv->dma_conf.dma_tx_size) {
		...
		np = STMMAC_NEXT_ENTRY(entry, priv->dma_conf.dma_tx_size);
		if (priv->extend_desc) {
...
static inline void stmmac_rx_refill(struct stmmac_priv *priv, u32 queue)
{
	...
	while (dirty-- > 0) {
		...
		np = STMMAC_NEXT_ENTRY(entry, priv->dma_conf.dma_rx_size);
		if (priv->extend_desc) {
...
static bool stmmac_rx_refill_zc(struct stmmac_priv *priv, u32 queue, u32 budget)
{
	...
	while (budget-- > 0 && entry != rx_q->cur_rx) {
		...
		rx_next = STMMAC_NEXT_ENTRY(entry, priv->dma_conf.dma_rx_size);
		if (priv->extend_desc) {
